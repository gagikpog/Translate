...
namespace UIGL {
	...
	UiglEvent UiglMessageBox::MouseFunc(int button, int state, int ax, int ay)
	{
		...
		if (ax > X && ax < X + W && ay > Y && ay < Y + H)
		{
			...
			{
				Visible = 0;
				result.Event = UIGL::uiglEvents::uiglMsgCancel | UIGL::uiglEvents::uiglEraseMsg;
				result.Name = Name;
				return result;
			}
			...
			if (result.Event == UIGL::uiglEvents::uiglMouseLeftDown && result.Name == "ok")
			{
				Visible = 0;
				result.Name = Name;
				result.Event = UIGL::uiglEvents::uiglMsgYes | UIGL::uiglEvents::uiglEraseMsg;
				return result;
			}
			...
			if (result.Event == UIGL::uiglEvents::uiglMouseLeftDown && result.Name == "no")
			{
				Visible = 0;
				result.Name = Name;
				result.Event = UIGL::uiglEvents::uiglMsgNo | UIGL::uiglEvents::uiglEraseMsg;
				return result;
			}
